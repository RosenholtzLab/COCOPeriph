...
for ecc in test_eccs:
    ...
    if train_ecc == "baseline":
        val_results_path = f'{base_path}/baseline/coco_val_2017_ecc{ecc}_eval/val_results.pkl'
    else:
        val_results_path = f'{base_path}/finetune_ecc{train_ecc}/coco_val_2017_ecc{ecc}_eval/val_results.pkl'
    if(os.path.exists(val_results_path)):
        with open(val_results_path, 'rb') as f:
            val_results = pickle.load(f)
    else:
        if train_ecc == "baseline": 
            evaluator = COCOEvaluator(f"coco_val_2017_ecc{ecc}", output_dir=f'{base_path}/baseline/coco_val_2017_ecc{ecc}_eval')
        else:
            evaluator = COCOEvaluator(f"coco_val_2017_ecc{ecc}", output_dir=f'{base_path}/finetune_ecc{train_ecc}/coco_val_2017_ecc{ecc}_eval')
